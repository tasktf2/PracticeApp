...
import com.setjy.practiceapp.presentation.base.recycler.bottom_sheet_fragment.BottomSheetFragment
import com.setjy.practiceapp.presentation.model.MessageUI
import com.setjy.practiceapp.presentation.view.channels.StreamListFragment
...
class TopicFragment : Fragment(R.layout.fragment_topic) {
    ...
    private val getMessagesOnLaunchUseCase by lazy {
        (requireContext().applicationContext as ZulipApp).globalDI.getMessagesOnLaunchUseCase
    }
...
//        disposable += Repo.getMessagesOnLaunch(streamName, topicName)
        disposable += getMessagesOnLaunchUseCase.execute(
            GetMessagesOnLaunchUseCase.Params(
                streamName = streamName,
                topicName = topicName
            )
        )
